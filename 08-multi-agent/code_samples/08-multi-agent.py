...
核心点在于4个prompt的设计，分别描述Reviewer Agent, Writer Agent的角色设定，
以及对话的终止规则、参与者选择规则。
...
def _create_kernel_from_chat_completion() -> Kernel:
    kernel = Kernel()
...
async def main():
    ...
    agent_reviewer = ChatCompletionAgent(
        kernel=_create_kernel_from_chat_completion(),
        name=REVIEWER_NAME,
    ...
    termination_function = KernelFunctionFromPrompt(
        ...
        Look for phrases like "approved", "the recommendation is approved", or any clear indication that the Concierge is satisfied with the suggestion.
    ...
    selection_function = KernelFunctionFromPrompt(
        ...
        No participant should take more that one turn in a row.
